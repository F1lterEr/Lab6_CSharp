...
class Animal {
    ...
    public virtual void Eat() {
        Console.WriteLine($"{name} такое-то животное ест " + food);
    }

    public virtual void Sleep() {
        Console.WriteLine($"{name} такое-то животное спит в " + location);
    }
}
...
class Dog : Animal {
    ...
    public override void MakeNoise() {
        Console.WriteLine($"Собака {name} лает");
    }
...
public class Program
{
    ...
    {
        Cat cat = new Cat("Кошка", "корм", "доме", "дымчатый");
        Dog dog = new Dog("Собака", "корм", "доме", true);
        Horse horse = new Horse("Лошадь", "яблоко", "конюшне", "Иванов С.А.");
